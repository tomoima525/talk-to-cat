...
export function useWebRTC(onMessage: (message: Message) => void): UseWebRTCResult {
  ...
  const [connectionQuality, setConnectionQuality] = useState<"excellent" | "good" | "fair" | "poor" | "unknown">(
    "unknown"
  );
   ...
   */
  const addLog = useCallback((direction: "SEND" | "RECV", type: string, message: Message) => {
    const log: DebugLog = {
