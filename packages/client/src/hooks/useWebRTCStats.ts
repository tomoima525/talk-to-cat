...
export function useWebRTCStats(peerConnection: RTCPeerConnection | null, isConnected: boolean): WebRTCStats {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const bitrateIn = previousBytesReceived > 0 ? ((bytesReceived - previousBytesReceived) * 8) / timeDelta : 0;
        const bitrateOut = previousBytesSent > 0 ? ((bytesSent - previousBytesSent) * 8) / timeDelta : 0;
