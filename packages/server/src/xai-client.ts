...
import { handleFunctionCallArguments, toolings } from "./function-callings";
  ...
  private async handleMessage(data: WebSocket.Data): Promise<void> {
    try {
      ...
      if (message.type === "response.function_call_arguments.done") {
        console.log(`[${sessionId}] 🔧 DEBUG: Function call arguments:`, message);
        await handleFunctionCallArguments(message, (msg) => this.sendMessage(msg));
      }
      ...
      // Handle errors
      if (eventType === "error") {
        console.error(`[${sessionId}] ❌ XAI API Error:`, message.error);
